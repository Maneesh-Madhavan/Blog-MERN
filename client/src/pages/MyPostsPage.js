...
export default function MyPostsPage() {
  ...
  if (!posts.length) return <p style={{textAlign:"center", marginTop:50}}>You have no posts yet.</p>;
    ...
    <div style={{maxWidth:900, margin:"0 auto", padding:"10px"}}>
      ...
      {posts.map(post => (
        <div key={post._id} style={{
          ...
          <Link to={`/post/${post._id}`} style={{textDecoration:"none", color:"#111", flex:1}}>
          ...
          <div style={{display:"flex", gap:10}}>
            ...
            <button onClick={() => deletePost(post._id)} title="Delete" className="icon-link">
              <MdDelete size={22} />
            </button>
          </div>
        </div>
